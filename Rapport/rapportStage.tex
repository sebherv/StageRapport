...
\chapter{Technologies, Contraintes et Plan de Projet}
	...
	\section{Capteurs}
		...
		\subsection{LIDAR Hokuyo UTM-30LX-EW}
		Le Hokuyo UTM-30-LX-EW est le type de LIDAR qui était utilisé dans Symeter V1, et sera donc ré-utilisé pour Symeter V2. C'est un équipement robuste et précis qui est de plus bien connu par Tellus Environment.
		
		\paragraph*{} Cet équipement est basé sur un laser infrarouge (longueur d'onde de 905nm) pour scanner un champ semi-circulaire de 270$\degres$, par pas de 0,25$\degres$. Il mesure la distances des objets à sa portée pour chaque incrément. La distance de détection maximale est de 30m.
		\newline
	...
	\section{Les grandes parties du projet}
...
\todoRediger{introduction du chapitre simulation tracteur}
	...
	\todoRediger{Qu'est ce que Gazebo, à quoi ça sert}
		\subsection{Construction d'un robot virtuel}
		\todoRediger{Décrire les éléments nécessaires à la description du robot}
		\todoRediger{URDF et SDF, world, contrôleurs}
		
		
		\subsection{Vérification de disponibilité des capteurs}
		\todoRediger{Besoin de vérifier la disponibilité et la performance des capteurs cibles de Symeter V2}
		
	\section{Contraintes de mise en oeuvre}
		\subsection{Quelques bugs génants}
		\todoRediger{Lister les problèmes et bugs qui ont nécessité la mise en place de contournements}
		\subsection{Simulation mécanique, frottements, adhérence}
		\todoRediger{La nature des "matériaux" utilisés dans gazebo est très "rigide": les chocs ne sont pas amortis.}
...
\chapter{Mise en place du processus de localisation}
	...
	\todoRediger{Exposer la problématique: fusionner des données avec potentiellement des fréquences d'échantillonnage différentes}
	
	\todoRediger{déterminer la pose du tracteur à partir des différents capteurs}
	
	\section{Filtres de Kalman}
	\todoRediger{théorie des filtres de Kalman}
	...
	\todoRediger{Utilisation du module robot\_localization}
	
	
\chapter{Exploitation des données LIDAR}

	\todoRediger{Exposer le plan du chapitre}
	\todoRediger{quelle est la qualité de mesure d'un LIDAR sur une surface potentiellement inclinée?}
	...
	\missingfigure{Inclinaison LIDAR par rapport au tracteur}
	
	\todoRediger{Biblio sur l'influence de l'inclinaison du lidar par rapport à la surface}
	\todoRediger{arg1}
	
	\section{Acquisition des données LIDAR}
	

	\todoRediger{mettre en forme les données lidar de manière à pouvoir les utiliser}
	\todoRediger{réduire la bande passante, ne garder que les données exploitatables}
		...
		\subsection{Filtrage de la ligne par traitement Voxel}
		\todoRediger{Enjeu: réduite la bande passante, ne pas sur-échantillonner au pied}
		
		\missingfigure{suréchantillonage au pied du tracteur, sous-échantillonnage sur les côtés.}
		
		\todoRediger{Exposer le principe du filtrage}
		
		\todoRediger{mise en oeuvre grace à la point cloud library}
		
	\section{Accumulation des nuages de points}
	\todoRediger{Objectif: reconstituer la scène à partir des nuages de points "ligne" obtenus à partir du processus d'acquisition}
	
		\subsection{Le principe de fonctionnement}
		\todoRediger{Garder en mémoire les nuages de points correspondant à des lignes précédemment acquise}
		\todoRediger{ne pas dupliquer les informations: éliminer les points doubles}
	...
	\todoRediger{implémentation d'un monde avec un silo et un tas d'ensilage}
	\todoRediger{capture à l'aide d'un lidar monté à l'arrière du tracteur}
	
		\subsection{Test sous gazebo}
		\todoRediger{Tracteur effectue une acquisition initiale de la scène}
...
\chapter{Mise en oeuvre à partir de mesures réelles}
	...
	\section{Exploitation des données}
		\todoRediger{données sans RTK --> besoin d'avoir un}
